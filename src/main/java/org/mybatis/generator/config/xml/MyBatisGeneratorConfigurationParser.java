...
public class MyBatisGeneratorConfigurationParser {
    ...
    protected void parseTable(Context context, Node node) {
        ...
        String insertBatchQueryId = attributes
                .getProperty("enableInsertBatch"); //$NON-NLS-1$
        if (stringHasValue(insertBatchQueryId)) {
            tc.setInsertBatchStatementEnabled(
                    isTrue(insertBatchQueryId));
        }
        
        
        
        String insertBatchSelectiveQueryId = attributes
                .getProperty("enableInsertBatchSelective"); //$NON-NLS-1$
        if (stringHasValue(insertBatchSelectiveQueryId)) {
            tc.setInsertBatchSelectiveStatementEnabled(
                    isTrue(insertBatchSelectiveQueryId));
        }
